...
public class DriverController extends DriverDo {
    ...
    public ResponseEntity<?> getDriverWithFilterWithOutFilter(
    ...
    public ResponseEntity<?> driverSignUp(@RequestParam(name = "name") String name,
        ...
        try {
            ...
            return new ResponseEntity<>(new DriverDto(driver.getId(), driver.getName(), "SignUp Successfully!"),
                    HttpStatus.CREATED);
        } catch (Exception e) {
    ...
    @PutMapping("/select")
    public ResponseEntity<?> selectCarByDriver(@RequestParam(required = false, name = "carId") Long carId,
            Principal principal) throws CarAlreadyInUseException {
        ...
        // DriverDto(driver.getId(),driver.getName(),"Driver selected car
        // Successfully!"), HttpStatus.OK);
        // } else {
        // throw new CarAlreadyInUseException("Car is is already used by other driver
        // with Id : " + driver.getId());
        // }
        if (selectCar(car, driver, driverService)) {
            return new ResponseEntity<>(
                    new DriverDto(driver.getId(), driver.getName(), "Driver selected car Successfully!"),
                    HttpStatus.OK);
        } else {
    ...
    public ResponseEntity<?> deselectCarByDriver(@RequestParam(name = "driverId") Long driverId) {
        ...
        try {
            driverService.save(driver);
            return new ResponseEntity<>(
                    new DriverDto(driver.getId(), driver.getName(), "Driver deselected car Successfully!"),
                    HttpStatus.OK);
        } catch (Exception e) {
            return new ResponseEntity<>("Error in saving the data", HttpStatus.INTERNAL_SERVER_ERROR);
        }
