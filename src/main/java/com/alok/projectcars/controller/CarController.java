...
public class CarController extends DriverController {
    ...
    public ResponseEntity<?> newCar(@RequestParam(required = false, name = "name") String name,
        ...
        if (roleOfCurrentUser.equals("ADMIN")) {
            Car car = new Car(null, name, model, licensePlate, seatCount, convertible, rating, engineType, manufacturer,
                    null, null, null);

            try {
                carService.save(car);
                response = new ResponseEntity<>(new CarDto(car.getId(), car.getName(), "Successfully Created!"),
                        HttpStatus.CREATED);
            } catch (Exception e) {
    ...
    public ResponseEntity<?> upgradeCar(@RequestParam(name = "id") Long id,
        ...
        if (roleOfCurrentUser.equals("ADMIN")) {
            ...
            try {
                carService.save(car);
                return new ResponseEntity<>(new CarDto(car.getId(), car.getName(), "Successfully record updated!"),
                        HttpStatus.OK);
            } catch (Exception e) {
